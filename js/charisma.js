(() => {
  ...
  {
    name: "Dishwasher",
    image: "https://stealthygundam.github.io/downgraded-doughnut/assets/characters/dishwasher.png",
  ...
  function showCharacterIntro() {
    ...
    gameDiv.innerHTML = `
      <h3>${npc.name}</h3>
      <img class="npc-image" src="${npc.image}">
      <p>${npc.backstory}</p>
      <h2 class="goal">Goal: ${npc.goal}</h2>
  ...
  function renderCheck() {
    ...
    if (checkIndex >= npc.checks.length) {
      ...
      if (npcIndex >= npcs.length) {
        gameDiv.innerHTML = `
          ...
          <p>You persuaded all NPCs successfully!</p>
    ...
    gameDiv.innerHTML = `<h3>${check.question}</h3>`;
  ...
  function handleChoice(success, npcName, tier) {
    gameDiv.innerHTML += `<h3>${reactionText(npcName, tier, success)}</h3>`;
